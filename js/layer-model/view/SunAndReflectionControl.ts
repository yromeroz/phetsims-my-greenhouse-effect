...
class SunAndReflectionControl extends Panel {
  ...
  public constructor( width: number, layersModel: LayerModelModel, tandem: Tandem ) {
    ...
    const titleText = new Text( MyGreenhouseEffectStrings.sunlightStringProperty, {
    ...
    const solarIntensitySliderLabel = new Text( MyGreenhouseEffectStrings.solarIntensityStringProperty, {
      font: HEADING_FONT
    } );
    ...
    _.times( majorTicksOnSolarIntensitySlider, index => {
      ...
      let labelText;
      if ( value === 1 ) {
        labelText = MyGreenhouseEffectStrings.ourSunStringProperty;
      }
      else {
        labelText = new PatternStringProperty(
          MyGreenhouseEffectStrings.valuePercentPatternStringProperty,
          { value: value * 100 }
        );
      }
    ...
    const surfaceAlbedoSliderLabel = new Text( MyGreenhouseEffectStrings.surfaceAlbedoStringProperty, {
      font: HEADING_FONT
    } );
...
myGreenhouseEffect.register( 'SunAndReflectionControl', SunAndReflectionControl );
