  ...
  public constructor( model: GreenhouseEffectModel,
    ...
    if ( options.useClippingFrame ) {
      ...
      const clippingFrame = new Path( clippingFramePath, {
        fill: MyGreenhouseEffectColors.screenBackgroundColorProperty
      } );
      this.addChild( clippingFrame );
    }

    // area between right edge of ScreenView and observation window
    const rightWidth = this.layoutBounds.right - MyGreenhouseEffectConstants.SCREEN_VIEW_X_MARGIN -
                       this.observationWindow.right - MyGreenhouseEffectConstants.OBSERVATION_WINDOW_RIGHT_SPACING;
    ...
    this.legendAndControlsVBox.leftTop = this.observationWindow.rightTop.plusXY(
      MyGreenhouseEffectConstants.OBSERVATION_WINDOW_RIGHT_SPACING,
      0
    );

    // Position the Reset All button.
    this.resetAllButton.right = this.layoutBounds.maxX - MyGreenhouseEffectConstants.SCREEN_VIEW_X_MARGIN;
...
myGreenhouseEffect.register( 'GreenhouseEffectScreenView', GreenhouseEffectScreenView );
