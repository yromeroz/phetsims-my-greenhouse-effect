...
class InstrumentVisibilityControls extends Panel {
  ...
  public constructor( model: LayersModel, providedOptions?: InstrumentVisibilityControlsOptions ) {
      ...
      ( netInflowOfEnergy, inRadiativeBalance, sunIsShining ) => {
        ...
        if ( sunIsShining ) {
          checkedUtterance.alert = StringUtils.fillIn(
            MyGreenhouseEffectStrings.a11y.energyBalanceCheckedPatternStringProperty,
            {
              checkedResponse: MyGreenhouseEffectStrings.a11y.energyBalanceCheckedAlertStringProperty,
          ...
          checkedUtterance.alert = MyGreenhouseEffectStrings.a11y.energyBalanceCheckedAlertStringProperty;
        }
      }
    );
    ...
    if ( model.energyBalanceVisibleProperty ) {
      children.push( new GreenhouseEffectCheckbox( model.energyBalanceVisibleProperty, MyGreenhouseEffectStrings.energyBalanceStringProperty, {
          // phet-io
          tandem: options.tandem.createTandem( 'energyBalanceCheckbox' ),

          // pdom
          helpText: MyGreenhouseEffectStrings.a11y.energyBalance.helpTextStringProperty,
          checkedContextResponse: checkedUtterance,
          uncheckedContextResponse: MyGreenhouseEffectStrings.a11y.energyBalanceUncheckedAlertStringProperty
        }
      ) );
    }
    if ( options.includeFluxMeterCheckbox ) {
      children.push(
        new GreenhouseEffectCheckbox(
          ...
          MyGreenhouseEffectStrings.fluxMeter.titleStringProperty,
          {
            // phet-io
...
myGreenhouseEffect.register( 'InstrumentVisibilityControls', InstrumentVisibilityControls );
