...
// constants
const AT_EQUILIBRIUM_THRESHOLD = MyGreenhouseEffectQueryParameters.atEquilibriumThreshold; // in Watts per square meter, empirically determined
...
// determined.
const EQUILIBRATION_TIME = MyGreenhouseEffectQueryParameters.atEquilibriumTime;
...
const SURFACE_DIMENSIONS = MyGreenhouseEffectConstants.SUNLIGHT_SPAN;
...
class EnergyAbsorbingEmittingLayer extends PhetioObject {
  ...
  public interactWithEnergy( emEnergyPackets: EMEnergyPacket[], dt: number ): void {
    ...
    const radiatedEnergyPerUnitSurfaceArea = Math.pow( this.temperatureProperty.value, 4 ) *
                                             MyGreenhouseEffectConstants.STEFAN_BOLTZMANN_CONSTANT * dt;
    ...
    if ( totalRadiatedEnergyThisStep > 0 ) {
      if ( this.substance.radiationDirections.includes( EnergyDirection.DOWN ) ) {
        emEnergyPackets.push( new EMEnergyPacket(
          MyGreenhouseEffectConstants.INFRARED_WAVELENGTH,
...
myGreenhouseEffect.register( 'EnergyAbsorbingEmittingLayer', EnergyAbsorbingEmittingLayer );
