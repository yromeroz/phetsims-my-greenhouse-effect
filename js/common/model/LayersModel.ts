...
const SUNLIGHT_SPAN = MyGreenhouseEffectConstants.SUNLIGHT_SPAN;
...
const DEFAULT_TEMPERATURE_UNITS = mapLetterToTemperatureUnits.get( MyGreenhouseEffectQueryParameters.defaultTemperatureUnits! );
...
class LayersModel extends GreenhouseEffectModel {
  ...
  public constructor( tandem: Tandem, providedOptions?: LayersModelOptions ) {
    ...
    this.surfaceTemperatureCelsiusProperty = new DerivedProperty(
      ...
      MyGreenhouseEffectUtils.kelvinToCelsius
    );

    this.surfaceTemperatureFahrenheitProperty = new DerivedProperty(
      [ this.surfaceTemperatureKelvinProperty ],
      MyGreenhouseEffectUtils.kelvinToFahrenheit
    );
...
myGreenhouseEffect.register( 'LayersModel', LayersModel );
